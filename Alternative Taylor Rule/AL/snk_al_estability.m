function [hee] = snk_al_estability(theta)
...
%hee.C_bar: HEE expressions for the coefficients on agent-type C's aggregate supply shock regressor
...
%Matrices
D1 = [1+(sigma^-1)*i_x,(sigma^-1)*i_pi;-kappa,1];
D2 = [alpha_A,(sigma^-1)*alpha_A;0,beta*alpha_A];
D3 = [alpha_B,(sigma^-1)*alpha_B;0,beta*alpha_B];
D4 = [(1-alpha_A-alpha_B),(sigma^-1)*(1-alpha_A-alpha_B);0,beta*(1-alpha_A-alpha_B)];
D5 = [1;0];
D6 = [0;1];
D7 = [-(sigma^-1);0];
...
if test_1 ~= 0 %Will be true if the equilibrium is unique
    ...
    test_2 = e_stability < 0; %All values will equal 1 if all of the real parts are negative, 0 otherwise
