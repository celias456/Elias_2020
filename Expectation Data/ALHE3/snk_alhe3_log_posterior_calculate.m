...
% A: beliefs for agent-type A
% B: beliefs for agent-type B
% C: beliefs for agent-type C
...
%Get value of the composite parameter alpha
alpha_A = theta(14);
alpha_B = theta(15);
...
[~,~,~,~,~,~,t,Sigma_epsilon,solution] = snk_re_build_state_space_matrices(number_endogenous_variables,number_jumper_variables_economy_wide,number_exogenous_variables,number_observed_variables,theta);

%Determine if the HEE solution is unique and stable
[hee] = snk_alhe3_estability(theta);
hee_test = sum(hee.e_stability);

%Determine if composite parameter alpha is less than 1
alpha_test = 0;
if alpha <= 1
    alpha_test = 1;
end

%Only calculate value of log posterior if the REE solution is unique and stable and if the HEE solution is unique and stable and if the composite parameter alpha is less than 1
if solution == 1 && hee_test == 2 && alpha_test == 1
    ...
    [J_1,J_c,J_2,Psi_L_0,Psi_L_1,Psi_L_2] = snk_alhe3_build_state_space_matrices(number_endogenous_variables,number_jumper_variables_agent_type,number_exogenous_variables,number_observed_variables,theta,A(:,:,1),B(:,:,1),C(:,:,1));
    ...
    for index_1 = first_observation:T
        ...
        [J_1,J_c,J_2,Psi_L_0,Psi_L_1,Psi_L_2] = snk_alhe3_build_state_space_matrices(number_endogenous_variables,number_jumper_variables_agent_type,number_exogenous_variables,number_observed_variables,theta,A(:,:,index_1),B(:,:,index_1),C(:,:,index_1));
    end
